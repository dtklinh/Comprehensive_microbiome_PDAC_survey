...
LP without accounting for environment

```{r}
rich_meta <- merge(pseq %>% sample_data(), pseq %>% estimate_richness(), by = "row.names")
model_lm <- lm(Shannon ~ LP, data = rich_meta)
summary(model_lm)
```

LP after accounting for environment

```{r}
model_lmer <- lmer(Shannon ~ LP + (1 | Environment), data = rich_meta)
summary(model_lmer)
```
