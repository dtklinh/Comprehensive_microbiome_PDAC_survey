...
pairwise_WT_Method <- function(df_longer, metric = "Composite_Score", strata = "Method", alternative = "less", m_order = c("Nj", "Restrictive", "SCRuB", "Decontam", "Raw")){
  ...
  res_upper <- pairwise_wilcox_test(data = df_longer, formula = as.formula("%s ~ %s", metric, strata), paired = TRUE, alternative = alternative, p.adjust.method = "BH") %>% 
    mutate(val = sprintf("%s(%s)", p.adj, p.adj.signif))

  df_longer[[strata]] <- factor(df_longer[[strata]], levels = rev(m_order))
  res_lower <- pairwise_wilcox_test(data = df_longer, formula = as.formula("%s ~ %s", metric, strata), paired = TRUE, alternative = alternative, p.adjust.method = "BH") %>% 
    mutate(val = sprintf("%s(%s)", p.adj, p.adj.signif))
  grid <- expand.grid(
    group1 = all_groups,
    group2 = all_groups,
...
```{r}
pairwise_WT_Method(df_all, alternative = "greater")
```
