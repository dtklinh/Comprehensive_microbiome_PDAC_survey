---
title: "Supplementary Figures for Manuscript"
author: "Linh Dang"
editor: visual
execute:
  warning: false
...
BetaPlot <- function(PhyloObjct, strata_f , n_per = 9999, title_method = "Original", dis_method = "bray", ordination_method = "PCoA"){
  ...
  bray_dist = phyloseq::distance(PhyloObjct, method=dis_method, binary = (dis_method=="jaccard"))
...
m_strata <- "year"
lst <- pseq %>% 
  AlphaPlotWrapper_Violin(strata = m_strata)
plt <- lst$Observed + lst$Shannon + lst$InvSimpson + plot_layout(guides = "collect") +
  plot_annotation(title = sprintf("Alpha diversity stratified by %s", m_strata)) &
  theme(plot.title = element_text(size = 16, face = "bold", hjust = 0.5))
## save plt to png
ggsave("./Figures_for_Manuscript_files/saved_png/S1.A.png", plt, width = 8, height = 6, dpi = 300)
print(plt)
```
...
plt <- lst$Observed + lst$Shannon + lst$InvSimpson + plot_layout(guides = "collect") +
  ...
  theme(plot.title = element_text(size = 16, face = "bold", hjust = 0.5))
## save plt to png
ggsave("./Figures_for_Manuscript_files/saved_png/S1.B.png", plt, width = 8, height = 6, dpi = 300)
plt
```
...
m_strata <- "person"
lst <- pseq %>% 
  ps_filter(person %in% c("CD", "CH", "J", "N", "TR", "U")) %>% 
  AlphaPlotWrapper_Violin(strata = m_strata)
plt <- lst$Observed + lst$Shannon + lst$InvSimpson + plot_layout(guides = "collect") +
  plot_annotation(title = sprintf("Alpha diversity stratified by %s", m_strata)) &
  theme(plot.title = element_text(size = 16, face = "bold", hjust = 0.5))
## save plt to png
#ggsave("./Figures_for_Manuscript_files/saved_png/S1.1.png", plt, width = 8, height = 6, dpi = 300)
plt
```
...
plt_wrench <- pseq %>% 
  ...
  BetaPlot(strata_f = "sample_type", dis_method = "jaccard", ordination_method = "PCoA", title_method = "NCT samples stratified by control types \n Wrench normalization - Jaccard")
plt_rar <- pseq %>% 
  rarefy_even_depth() %>% 
  BetaPlot(strata_f = "sample_type", dis_method = "jaccard", ordination_method = "PCoA", title_method = "NCT samples stratified by control types \n Rarefaction - Jaccard")
...
#require(patchwork)
grp <- "year"
dis_method <- "bray" ## jaccard
plt_wrench <- pseq %>% 
  WrenchWrapper(grp = grp) %>% 
  BetaPlot(strata_f = grp, dis_method = dis_method, ordination_method = "PCoA", title_method = sprintf("NCT samples stratified by %s \n Wrench normalization - %s", grp, dis_method))
plt_rar <- pseq %>% 
  rarefy_even_depth() %>% 
  BetaPlot(strata_f = grp, dis_method = dis_method, ordination_method = "PCoA", title_method = sprintf("NCT samples stratified by %s \n Rarefaction - %s", grp, dis_method))
plt <- plt_wrench + plt_rar + plot_layout(guides = "collect")
## save to png
#ggsave("./Figures_for_Manuscript_files/saved_png/1.3.png", plot = plt, width = 8, height = 6, dpi = 300)
plt
...
#| fig-height: 7
#| fig-lightbox: true
plt <- CompositionalPlot_Viz_v02(pseq, supper_rank = "phylum", smaller_rank = "family", target_taxa_name = "Sphingomonadaceae", supper_rank_list = c("Pseudomonadota", "Bacillota", "Actinomycetota"), facet = "season", each_num = 4, tax_order_by = sum, my_title = "Order by sum")
## save to png
...
**Figure S1.9**: Stacked barplot of bacterial composition in negative control samples at phylum-family level, stratified by season.
...
plt <- pseq %>% 
  ps_filter(person %in% c("CD", "CH", "J", "N", "TR", "U")) %>% 
  CompositionalPlot_Viz_v02(supper_rank = "phylum", smaller_rank = "family", target_taxa_name = "Sphingomonadaceae", supper_rank_list = c("Pseudomonadota", "Bacillota", "Actinomycetota"), facet = "person", each_num = 4, tax_order_by = sum, my_title = "Order by sum")
## save to png
#ggsave("./Figures_for_Manuscript_files/saved_png/1.5.png", plot = plt, width = 8, height = 6, dpi = 300)
# png("./Figures_for_Manuscript_files/saved_png/1.5.png", width = 2400, height = 1800, res = 300)
# 
...
**Figure 3.1+**: Normality Check with QQ-Plot
