...
pseq <- readRDS("./Rscripts/TTDresden/pseq.rds")
...
## remove noise, trivial noise both in true and nct samples : DONE
pseq_true <- pseq %>% 
  microViz::ps_filter(TRUE.CTRL == "TrueSample")
pseq_NCT <- pseq %>% 
  ps_filter(TRUE.CTRL == "CTRL")
...
saveRDS(pseq_true_restrictive, "./Rscripts/TTDresden/pseq_restrictive.rds")

## decontam
library(decontam)
pseq <- pseq %>% 
  ps_mutate(is.neg = ifelse(TRUE.CTRL == "TrueSample", FALSE, TRUE))
...
saveRDS(pseq_true_decontam, "Rscripts/TTDresden/pseq_decontam.rds")
...
s_metadata <- pseq %>% 
  meta() %>% 
  mutate(is_control = if_else(TRUE.CTRL == "TrueSample", F, T)) %>% 
  select(is_control, sample_type = ControlType) %>% 
  mutate(sample_type = replace_na(sample_type, "true_sample"))
s_out <- SCRuB(s_data, s_metadata, c("buffer", "PCR"))
...
pseq_true_scrub <- pseq_true_scrub %>% 
  ...
  tax_filter(min_prevalence = 1,
             prev_detection_threshold = 2,
             min_total_abundance = 1e-6)
saveRDS(pseq_true_scrub, "./Rscripts/TTDresden//pseq_SCRuB.rds")
...
df1 <- stat_test(pseq_t = pseq_true_Nj, pseq_n = pseq_NCT, NCT_batch = list("ControlType" = c("buffer", "PCR")))
# df2 <- stat_test(pseq_t = pseq_true_Nj, pseq_n = pseq_NCT, NCT_batch = list("ControlType" = c("buffer", "seq")))
# df3 <- stat_test(pseq_t = pseq_true_Nj, pseq_n = pseq_NCT, NCT_batch = list("ControlType" = c("pcr", "buffer", "seq")), prefix = "All")
# df_all <- df1 %>% 
...
saveRDS(pseq_true_Nj, "./Rscripts/TTDresden/pseq_Nejman.rds")
