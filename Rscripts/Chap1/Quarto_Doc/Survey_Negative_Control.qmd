...
##----------------
format_pairwise_adonis2 <- function(adonis_pw) {
  nms <- attributes(adonis_pw)[["names"]]
  nms <- nms[2:length(nms)]
  df_res <- data.frame(Pair = character(0), R2 = double(0), F = double(0), p_val = double(0))
  for(nm in nms){
    df_res[nrow(df_res)+1,] <- c(nm, adonis_pw[[nm]][["R2"]][[1]], 
                                 adonis_pw[[nm]][["F"]][[1]],
                                 adonis_pw[[nm]][["Pr(>F)"]][[1]])
  }
  return(df_res)
...
##--
AlphaPlotWrapper_Violin <- function(PhyloObj, strata = "sample_type", roundUp = TRUE){
  ...
  plt.1 <- AlphaPlot_Violin(PhyloObj, index = "Observed", strata = strata, y_label = "Observed species", add_legend = T)
  plt.2 <- AlphaPlot_Violin(PhyloObj, index = "Shannon", strata = strata, y_label = "Shannon index", add_legend = T)
  plt.3 <- AlphaPlot_Violin(PhyloObj, index = "InvSimpson", strata = strata, y_label = "Inv Simpson index", add_legend = T)
...
The prior and trivial filtering as following:

-   remove all samples whose number of reads are less than 500

-   remove all taxa as follows:

    -   detection of prevalence is at least 2 reads, and prevalence at least in 1 samples.

    -   minimum total abundance is 0.000001 (or 19 reads in this project).

```{r}
pseq <- rprojroot::find_rstudio_root_file() %>% 
  #file.path("data/Chap1/NCTs_v7.rds") %>% 
  file.path("data/Chap1/NCT_2026April_v1.rds") %>% 
  readRDS()
## remove two few reads sample and noisy tax
## for the moment, only consider control type: Buffer, Paraffin, PCR, Sequencing
N <- 500
pseq <- pseq %>% 
...
As a results, we have `{R} phyloseq::nsamples()` samples, and `{R} phyloseq::ntaxa(pseq)` taxa.

```{r}
meta(pseq)$Control_type %>% table() %>% print()
```

Add year info to metadata
...
```{r}
plt1 <- pseq %>% 
  AlphaPlot_Violin(index = "Observed", strata = "Control_type", y_label = "Observed species", add_legend = T)
plt1
```
...
lst <- pseq %>% 
  ...
  AlphaPlotWrapper_Violin(strata = "Control_type")
lst$Observed + lst$Shannon + lst$InvSimpson + plot_layout(guides = "collect") +
  plot_annotation(title = "Alpha diversity stratified by control types - Wrench normalization") &
  theme(plot.title = element_text(size = 16, face = "bold", hjust = 0.5))
```

## 0.3 With rarefaction
...
lst <- pseq %>% 
  ...
  AlphaPlotWrapper_Violin(strata = "Control_type")
lst$Observed + lst$Shannon + lst$InvSimpson + plot_layout(guides = "collect") +
  plot_annotation(title = "Alpha diversity stratified by control types - Rarefaction") &
  theme(plot.title = element_text(size = 16, face = "bold", hjust = 0.5))
```

## **Stratified by other factor**

### By Year

Use rarefaction for simplicity.
...
lst <- pseq_tmp %>% 
  ps_filter(!is.na(Year)) %>% 
  rarefy_even_depth() %>% 
  AlphaPlotWrapper_Violin(strata = "Year")
lst$Observed + lst$Shannon + lst$InvSimpson + plot_layout(guides = "collect") +
  plot_annotation(title = "Alpha diversity stratified by year") &
  theme(plot.title = element_text(size = 16, face = "bold", hjust = 0.5))
```
...
## limit to J, N, U
lst <- pseq %>% 
  #ps_filter(Person != "unknown") %>% 
  ps_filter(Person %in%  c("J", "N", "U")) %>% 
  rarefy_even_depth() %>% 
  AlphaPlotWrapper_Violin(strata = "Person")
lst$Observed + lst$Shannon + lst$InvSimpson + plot_layout(guides = "collect") +
  plot_annotation(title = "Alpha diversity stratified by lab personals") &
  theme(plot.title = element_text(size = 16, face = "bold", hjust = 0.5))
```
...
Bray distance with PCoA, Wrench normalization and rarefaction
...
require(patchwork)
plt_wrench <- pseq %>% 
  ps_filter(Person %in% c("J", "U", "N")) %>% 
  WrenchWrapper(grp = "Person") %>% 
  BetaPlot(strata_f = "Person", dis_method = "bray", ordination_method = "PCoA", title_method = "NCT samples stratified by lab personals - Wrench normalization")
plt_rar <- pseq %>% 
  ps_filter(Person %in% c("J", "U", "N")) %>% 
  rarefy_even_depth() %>% 
  BetaPlot(strata_f = "Person", dis_method = "bray", ordination_method = "PCoA", title_method = "NCT samples stratified by lab personals - Rarefaction")
plt_wrench + plt_rar + plot_layout(guides = "collect")
```

Pairwise, with and without accounting for technician

CLR transformation with Euclidean distance

pairwise Control_type without accounting for lab personals
...
dist_matrix <- pseq_tmp %>% 
  #ps_filter(Person %in% c("J", "U", "N")) %>% 
  microbiome::transform(transform = "clr") %>% 
  phyloseq::distance(method = "euclidean")
metadata <- meta(pseq_tmp)
#perm <- how(blocks = metadata$TA, nperm = 9999)
adonis_res <- pairwise.adonis2(dist_matrix ~ Control_type, 
                      data = metadata, 
                      permutations = 9999
                      #permutations = perm
                      )

print(format_pairwise_adonis2(adonis_res))
```

Pairwise Control_type accounting for lab personal
...
# check
stopifnot(
  all(attr(dist_matrix, "Labels") == rownames(metadata))
)
perm <- how(nperm = 9999)
setBlocks(perm) <- with(metadata, Person)
adonis_res <- pairwise.adonis2(dist_matrix ~ Control_type, 
                      data = metadata, 
                      strata = 'Person',
                      nperm  = 9999)
...
Bray with rarefaction, without any constraints
...
dist_matrix <- pseq %>% 
  ...
  phyloseq::distance(method = "bray")
metadata <- meta(pseq)
#perm <- how(blocks = metadata$Person, nperm = 9999)
adonis_res <- pairwise.adonis2(dist_matrix ~ Control_type, 
                               strata = "Person",
                                data = metadata,
                               nperm = 9999
...
#| fig-height: 8
#| fig-lightbox: true
CompositionalPlot_microViz(pseq, target_rank = "class", facet = "Control_type", num = 15, tax_order = sum, my_title = "Tax order by sum")
```
...
#| fig-height: 7
#| fig-lightbox: true
CompositionalPlot_Viz_v02(pseq, grp = "Control_type", supper_rank = "phylum", smaller_rank = "family", target_taxa_name = "Sphingomonadaceae", supper_rank_list = c("Pseudomonadota", "Bacillota", "Actinomycetota"), facet = "sample_type", each_num = 4, tax_order_by = sum, my_title = "Order by sum")
```
...
#| fig-height: 7
#| fig-lightbox: true
CompositionalPlot_Viz_v02(pseq, grp = "Control_type", supper_rank = "phylum", smaller_rank = "family", target_taxa_name = "Sphingomonadaceae", supper_rank_list = c("Pseudomonadota", "Bacillota", "Actinomycetota"), facet = "sample_type", each_num = 4, tax_order_by = prev, my_title = "Order by prevalence")
```

### Grouped by lab personals
...
pseq %>% 
  ps_filter(Person %in% c("J", "N", "U")) %>% 
  CompositionalPlot_Viz_v02(grp = "Person", supper_rank = "phylum", smaller_rank = "family", target_taxa_name = "Sphingomonadaceae", supper_rank_list = c("Pseudomonadota", "Bacillota", "Actinomycetota"), facet = "sample_type", each_num = 4, tax_order_by = sum, my_title = "Order by sum")
```
...
pseq %>% 
  ps_mutate(Year = as.character(lubridate::year(Seq_date))) %>% 
  CompositionalPlot_Viz_v02(grp = "Year", supper_rank = "phylum", smaller_rank = "family", target_taxa_name = "Sphingomonadaceae", supper_rank_list = c("Pseudomonadota", "Bacillota", "Actinomycetota"), facet = "sample_type", each_num = 4, tax_order_by = sum, my_title = "Order by sum")
```

## Heatmap

Top 15 genera order by abundance
...
Top 15 genera order by prevalence
...
Using MaAsLin2, Aldex2 and ANCOM-BC2 to find the d.a species among control types
...
m_rank <- "class"
#sample_data(pseq)$Control_type <- 
res <- pseq %>% 
  #ps_filter(person != "unknown") %>%
  #ps_filter(Control_type != "Sequencing") %>%
  #tax_filter(min_prevalence = 0.05, min_total_abundance = 5e-6, prev_detection_threshold = 2) %>%
  #tax_agg(rank = m_rank) %>%
  ancombc2(
  data = .,
  tax_level = m_rank,
  fix_formula = "Person + Control_type",
  #rand_formula = "(1|Person)",
  pairwise = TRUE,
  p_adj_method = "BH",
  group = "Control_type",         # Variable for multi-group comparison/structural zeros
...
# ancombc2_genus.rds: --> rank: genus, min_prevalence = 0.05, min_total_abundance = 1e-5
# ancombc2_species.rds --> rank: species, min_prevalence = 0.05, min_total_abundance = 1e-5, prev_detection_threshold = 2
# ancombc2_family.rds  --> rank: family, min_prevalence = 0.05, min_total_abundance = 1e-5, prev_detection_threshold = 2
saveRDS(res, "./ancombc2_class.rds")
```
...
```{r}
res <- readRDS("./ancombc2_species.rds")
res_prim <- res$res
sig_taxa_species <- res_prim %>%
...
# Plotting
ggplot(sig_taxa_species, aes(x = reorder(taxon, lfc_sample_typeparaffin_ctrl), 
                                    y = lfc_sample_typeparaffin_ctrl, fill = Direction)) +
...
```{r}
res <- readRDS("./ancombc2_genus.rds")
res_prim <- res$res
...
```{r}
res <- readRDS("./ancombc2_family.rds")
res_prim <- res$res
sig_taxa_family <- res_prim %>%
  mutate(Direction = if_else(lfc_sample_typeparaffin_ctrl >0, "Paraffin Control", "Buffer Control")) %>% 
  filter(p_sample_typeparaffin_ctrl <=0.05) #%>% 
  #arrange(desc(abs(lfc_sample_typeparaffin_ctrl))) %>% 
  #slice_head(n = 20)
...
# Plotting
ggplot(sig_taxa_family, aes(x = reorder(taxon, lfc_sample_typeparaffin_ctrl), y = lfc_sample_typeparaffin_ctrl, fill = Direction)) +
  scale_fill_discrete("") +
  geom_bar(stat = "identity") +
  coord_flip() +
  labs(title = "ANCOM-BC2 - Differential Abundance Analyses", x = "Families", y = "Log Fold Change") +
  theme_minimal()
```
...
plt_species <- ggplot(sig_taxa_species, aes(x = reorder(taxon, lfc_sample_typeparaffin_ctrl), y = lfc_sample_typeparaffin_ctrl, fill = Direction)) +
  scale_fill_discrete("") +
  geom_bar(stat = "identity") +
  coord_flip() +
  labs(x = "Species", y = "Log Fold Change") +
  theme_minimal()
plt_genus <- ggplot(sig_taxa_genus, aes(x = reorder(taxon, lfc_sample_typeparaffin_ctrl), y = lfc_sample_typeparaffin_ctrl, fill = Direction)) +
  scale_fill_discrete("") +
  geom_bar(stat = "identity") +
  coord_flip() +
  labs(x = "Genera", y = "Log Fold Change") +
  theme_minimal()
plt_family <- ggplot(sig_taxa_family, aes(x = reorder(taxon, lfc_sample_typeparaffin_ctrl), y = lfc_sample_typeparaffin_ctrl, fill = Direction)) +
  scale_fill_discrete("") +
  geom_bar(stat = "identity") +
  coord_flip() +
  labs(x = "Families", y = "Log Fold Change") +
  theme_minimal()


combined_plot <- (plt_family | plt_genus) / 
                  (plt_species | guide_area()) +
  plot_layout(guides = "collect") &
  theme(legend.position = "right")

combined_plot +
  plot_annotation(
    title = "ANCOM-BC2 - Differential Abundance Analyses",
...
#| include: false
library(Maaslin2)
fix_eff <- "Control_type"
#rnk_taxa <- "species"
outdir <- file.path("./MaAslin2_out/", "M1")
df_inputdata <- pseq %>% 
...
fix_eff <- "Control_type"
rnk_taxa <- "genus"
outdir <- file.path("./MaAslin2_out/", "M2")
df_inputdata <- pseq %>% 
  ps_filter(sample_type != "pcr_ctrl") %>% 
...
fix_eff <- "sample_type"
rnk_taxa <- "family"
outdir <- file.path("./MaAslin2_out/", "M3")
df_inputdata <- pseq %>% 
  ps_filter(sample_type != "pcr_ctrl") %>% 
  tax_agg(rank = rnk_taxa) %>% 
  abundances() %>% 
...
#| include: false
fix_eff <- "sample_type"
rnd_eff <- c("person", "year")
#rnk_taxa <- "family"
outdir <- file.path("./MaAslin2_out/", "C1")
...
rnd_eff <- c("person", "year")
rnk_taxa <- "genus"
outdir <- file.path("./MaAslin2_out/", "C2")
...
rnd_eff <- c("person", "year")
rnk_taxa <- "family"
outdir <- file.path("./MaAslin2_out/", "C3")
...
metadata <- pseq %>% 
  ...
  ps_filter(person != "unknown") %>% 
  microbiome::meta()
mm <- model.matrix(~ sample_type + person, data = metadata)
...
glm_result <- aldex.glm(x, mm, fdr.method = "BH")
...
res_tidy <- glm_result %>% 
  ...
  dplyr::select(Taxon,
         p_val = `sample_typeparaffin_ctrl:pval`,
         q_val = `sample_typeparaffin_ctrl:pval.padj`,
         estimate = `sample_typeparaffin_ctrl:Est`) %>% 
  mutate(is_sig = p_val <= 0.1)
top_hits <- res_tidy %>%
  filter(is_sig) %>%
  #arrange(p_val) %>%
...
ggplot(top_hits, aes(x = reorder(Taxon, estimate), y = estimate, fill = estimate > 0)) +
  geom_bar(stat = "identity") +
  coord_flip() +
  scale_fill_manual(values = c("skyblue", "salmon"), labels = c("Buffer Control", "Paraffin Control"), name = "Control") +
