...
res <- ancombc2(
  data = pseq ,#%>% tax_filter(min_prevalence = 0.1, min_total_abundance = 1e-6),
  tax_level = "genus",
  fix_formula = "Envir",
  rand_formula = "(1|TA)",
  p_adj_method = "BH",
  ...
  neg_lb = FALSE,                   # TRUE: Better structural zero detection for larger samples
...
saveRDS(res, "./ancombc2_fix_formular_TA.rds")
```
...
```{r}
# Filter for significant results
#res <- readRDS("./ancombc2_fix_formular_TA.rds")
res_prim <- res$res
sig_taxa <- res_prim %>%
  filter(p_TATA2 <=0.05) #%>% 
  #filter(diff_TATA2 == TRUE)# %>% # Replace ConditionName with your actual group
  #arrange(desc(abs(lfc_ConditionName))) %>%
  #head(20)

# Plotting
ggplot(sig_taxa, aes(x = reorder(taxon, lfc_TATA2), y = lfc_TATA2, fill = lfc_TATA2 > 0)) +
  geom_bar(stat = "identity") +
  coord_flip() +
  labs(title = "Differential Abundance by Techicians (Accounting for Condition)",
       x = "Taxon", y = "Log Fold Change") +
  theme_minimal()
```
