...
```{r}
pseq_raw <- pseq_raw %>% ps_mutate(Replica = "R1")
rep_raw <- rep_raw %>% ps_mutate(Replica = "R2")
merge_raw <- merge_phyloseq(pseq_raw, rep_raw)
dis_mx <- merge_raw %>% microbiome::transform(transform = "compositional") %>% phyloseq::distance(method = "bray")
