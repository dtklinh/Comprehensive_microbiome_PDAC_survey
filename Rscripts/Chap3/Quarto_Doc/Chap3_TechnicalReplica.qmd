...
pairwise_WT_Method <- function(df_longer){
  ...
  res_upper <- pairwise_wilcox_test(data = df_longer, formula = Val ~ Methods, paired = TRUE, alternative = "less", p.adjust.method = "BH") %>% 
    mutate(val = sprintf("%s(%s)", p.adj, p.adj.signif))

  df_longer$Methods <- factor(df_longer$Methods, levels = rev(m_order))
  res_lower <- pairwise_wilcox_test(data = df_longer, formula = Val ~ Methods, paired = TRUE, alternative = "less", p.adjust.method = "BH") %>% 
    mutate(val = sprintf("%s(%s)", p.adj, p.adj.signif))
  grid <- expand.grid(
    group1 = all_groups,
    group2 = all_groups,
...
```{r}
tab <- pairwise_WT_Method(df = df_num)
print(tab)
```
...
rep5 <- rep_Nj %>% 
  microViz::tax_filter(min_prevalence = min_prev, min_total_abundance = min_abund)
df_res <- funct_overlap(ps1, ps2, ps3, ps4, ps5,
                        rep1, rep2, rep3, rep4, rep5)
print(df_res)
```
...
rnk_tax <- "genus"
ps1 <- pseq_raw %>% 
  tax_agg(rank = rnk_tax) %>% 
  ps_get() %>% 
  microViz::tax_filter(min_prevalence = min_prev, min_total_abundance = min_abund, tax_level = rnk_tax)
ps2 <- pseq_restrictive %>% 
  tax_agg(rank = rnk_tax) %>% 
  ps_get() %>% 
  microViz::tax_filter(min_prevalence = min_prev, min_total_abundance = min_abund, tax_level = rnk_tax)
...
rep5 <- rep_Nj %>% 
  ...
  ps_get() %>% 
  microViz::tax_filter(min_prevalence = min_prev, min_total_abundance = min_abund, tax_level = rnk_tax)
df_res <- funct_overlap(ps1, ps2, ps3, ps4, ps5, rep1, rep2, rep3, rep4, rep5)
print(df_res)
```
...
**Sample-wise and p-value**

**Idea**: For each sample, each decontamination method, we calculate the overlap-overall ratio, then using paired Wilcox rank test for the assessment.

Calculate dataframe for analyses
...
df_num <- df_num %>% 
  ...
  select(AN_NR, Raw, Restrictive, Decontam, SCRuB, Nj)
df_longer <- df_num %>% pivot_longer(cols = -AN_NR, names_to = "Methods", values_to = "Ratio")
df_longer$Methods <- factor(df_longer$Methods, levels = c("Nj", "Restrictive", "SCRuB", "Decontam", "Raw"))
AlphaPlot_Violin_Wilcox(df=df_longer, SampleID = "AN_NR", strata = "Methods", val = "Ratio", y_label = "Methods", subtitle = "Higher is better", comparisons = list(c("Nj", "Restrictive"), c("Nj", "SCRuB"), c("Nj", "Decontam"), c("Nj", "Raw")), alternative = "greater", p.adjust.method = "BH")
```
