...
plt <- pseq2 %>%
  ...
  facet_wrap(facets = vars(!!sym(facet)), scales = "fixed") +
  coord_flip() +
  labs( x= "Sample ID", y = "Relative abundance",
        title = my_title,
        subtitle = my_subtitle) +
...
CompositionalPlot_Viz_v04 <- function(PhyloseqObj, 
  ...
  #   #ggtitle(paste0(my_title)) +
  #   #theme(legend.text = element_text(family = "mono")) # for text alignment
  #   facet_wrap(facets = my_facet, nrow = 1, scales = "free") +
  #   labs(x = NULL, y = NULL,
  #        title = my_title,
  #       subtitle = my_subtitle) +
  #   theme(axis.text.y = element_blank(),
  ...
  plt <- pseq2 %>%
    ...
    facet_wrap(facets = vars(!!sym(my_facet)), scales = "fixed") +
    coord_flip() +
    labs( x= "Sample ID", y = "Relative abundance",
          title = my_title,
          subtitle = my_subtitle) +
...
Re-arrange samples to the same order

```{r}
psLst <- list(Raw_R1 = pseq_raw, Raw_R2 = rep_raw,
...
for (i in 1:length(psLst)) {
  psLst[[i]] <- psLst[[i]] %>% ps_arrange(AN_NR)
}
## check
all(meta(psLst[["Nj_R1"]])$AN_NR == meta(psLst[["Nj_R2"]])$AN_NR)
```
...
df_longer$Methods <- factor(df_longer$Methods, levels = c("Nj", "Restrictive", "SCRuB", "Decontam", "Raw"))
AlphaPlot_Violin_Wilcox(df_longer, "AN_NR", "Methods", "Val", y_label = "bray distance", subtitle = "The lower the better", comparisons = list(c("Nj", "Restrictive"), c("Nj", "SCRuB"), c("Nj", "Decontam"), c("Nj", "Raw")), alternative = "less")
```
...
**Remarks**: could not make any conclusion.

#### P-value squared table

Pairwise p-value for each method

```{r}
#### create square table, where N_i,j tell us if Method i is less than Method j
pairwise_WT_Method <- function(df_longer, alternative = "less"){
  ## df: df_longer format --> AN_NR  Methods    Val
  m_order <- c("Nj", "Restrictive", "SCRuB", "Decontam", "Raw")
  all_groups <- m_order
  df_longer$Methods <- factor(df_longer$Methods, levels = m_order)
  res_upper <- pairwise_wilcox_test(data = df_longer, formula = Val ~ Methods, paired = TRUE, alternative = alternative, p.adjust.method = "BH") %>% 
    mutate(val = sprintf("%s(%s)", p.adj, p.adj.signif))

  df_longer$Methods <- factor(df_longer$Methods, levels = rev(m_order))
  res_lower <- pairwise_wilcox_test(data = df_longer, formula = Val ~ Methods, paired = TRUE, alternative = alternative, p.adjust.method = "BH") %>% 
    mutate(val = sprintf("%s(%s)", p.adj, p.adj.signif))
  grid <- expand.grid(
    group1 = all_groups,
    group2 = all_groups,
  ...
  full <- grid %>%
    left_join(res_upper %>% select(group1, group2, p_less = val), 
              by = c("group1", "group2")) %>%
    left_join(res_lower %>% select(group1, group2, p_greater = val), 
              by = c("group1", "group2"))
  full <- full %>%
...
```{r}
tab <- pairwise_WT_Method(df_longer, alternative = "less")
print(tab)
```

Note: value in the table \$M\_{i,j}\$ means if value from Method \$i\$ is less than value from Method \$j\$.
...
```{r}
df_longer$Methods <- factor(df_longer$Methods, levels = c("Nj", "Restrictive", "SCRuB", "Decontam", "Raw"))
AlphaPlot_Violin_Wilcox(df_longer, "AN_NR", "Methods", "Val", y_label = "b-ray distance", subtitle = "The lower is better", comparisons = list(c("Nj", "Restrictive"), c("Nj", "SCRuB"), c("Nj", "Decontam"), c("Nj", "Raw")), alternative = "less")
```

Pairwise p-value

```{r}
tab <- pairwise_WT_Method(df_longer)
print(tab)
```
...
df_longer$Methods <- factor(df_longer$Methods, levels = c("Nj", "Restrictive", "SCRuB", "Decontam", "Raw"))
AlphaPlot_Violin_Wilcox(df_longer, "AN_NR", "Methods", "Val", y_label = "b-ray distance", subtitle = "The lower is better", comparisons = list(c("Nj", "Restrictive"), c("Nj", "SCRuB"), c("Nj", "Decontam"), c("Nj", "Raw")), alternative = "less")
# res <- wilcox_test(data = df_longer, formula = Val ~ Methods, paired = TRUE, comparisons = list(c("Nj", "Restrictive"), c("Nj", "SCRuB"), c("Nj", "Decontam"), c("Nj", "Raw")), alternative = "less")
# print(res)
```
...
```{r}
dist_metric <- "jaccard"
trans <-"rarefaction"  #"rarefaction", identity, Wrench, clr, compositional
...
```{r}
df_longer$Methods <- factor(df_longer$Methods, levels = c("Nj", "Restrictive", "SCRuB", "Decontam", "Raw"))
AlphaPlot_Violin_Wilcox(df_longer, "AN_NR", "Methods", "Val", y_label = "Jarcard distance", subtitle = "The lower is better", comparisons = list(c("Nj", "Restrictive"), c("Nj", "SCRuB"), c("Nj", "Decontam"), c("Nj", "Raw")), alternative = "less", p.adjust.method = "BH")
```
...
```{r}
dist_metric <- "jaccard"
trans <-"identity"  #"rarefaction", Wrench
df <- process_replica(pseq_raw, rep_raw, normalize = trans, distance = dist_metric, col_name = "Raw") %>% 
  left_join(., process_replica(pseq_restrictive, rep_restrictive, normalize = trans, distance = dist_metric, col_name = "Restrictive"), by = "AN_NR") %>%
  left_join(., process_replica(pseq_decontam, rep_decontam, normalize = trans, distance = dist_metric, col_name = "Decontam"), by="AN_NR") %>% 
  left_join(., process_replica(pseq_SCRuB, rep_SCRuB, normalize = trans, distance = dist_metric, col_name = "SCRuB"), by="AN_NR") %>% 
...
```{r}
df_longer$Methods <- factor(df_longer$Methods, levels = c("Nj", "Restrictive", "SCRuB", "Decontam", "Raw"))
AlphaPlot_Violin_Wilcox(df_longer, "AN_NR", "Methods", "Val", y_label = "Jarcard distance", subtitle = "The lower the better", comparisons = list(c("Nj", "Restrictive"), c("Nj", "SCRuB"), c("Nj", "Decontam"), c("Nj", "Raw")), alternative = "less", p.adjust.method = "BH")
```
...
```{r}
## order: Nj, Restrictive, SCRuB, Decontam, Raw
df_longer$Methods <- factor(df_longer$Methods, levels = c("Nj", "Restrictive", "SCRuB", "Decontam", "Raw"))
AlphaPlot_Violin_Wilcox(df_longer, "AN_NR", "Methods", "Val", y_label = "Aitchison distance", subtitle = "The lower the better", comparisons = list(c("Nj", "Restrictive"), c("Nj", "SCRuB"), c("Nj", "Decontam"), c("Nj", "Raw")), alternative = "less", p.adjust.method = "BH")
```

#### P-value squared table

Pairwise p-value for decontamination methods
...
(u1 | u2_blurred) + plot_layout(guides='collect')
```
...
ps_all_merged %>% 
  CompositionalPlot_Viz_v04(supper_rank = "phylum", smaller_rank = "family", n_super_rank = 3, my_facet = "Group", each_num = 4, tax_order_by = sum, tax_trans = "identity", my_title = "Compositional Barplot", my_subtitle = "Raw vs Nj")
```
...
### 3.2 Inspect number of each methods in each replica

#### Overlap-Overall Ratio

For each list of phyloseq objects, we calculate the table of raio. Afterwards, calculating the ratio for each sample and then p-value from pair Wilcox test. Pairwise directional comparation as well.

Calculation the ratio between intersection and union

Prepare necessary functions

```{r}
#### create overal table of overlap-overall ratio
Ratio_table_calc <- function(psLst){
  Pair_names <- list(c("Raw_R1","Raw_R2"), c("Restrictive_R1","Restrictive_R2"),
                       c("Decontam_R1", "Decontam_R2"), c("SCRuB_R1", "SCRuB_R2"),
                       c("Nj_R1", "Nj_R2"))
  df_ratio <- data.frame(n_rep1 = numeric(), n_rep2 = numeric(), n_intersect = numeric(),
                         n_union = numeric(), Ratio = numeric())
  for (pair in Pair_names) {
      col1 <- pair[1]
      col2 <- pair[2]
      nm <- strsplit(col1, split = "_")[[1]][1]
      df_vec <- data.frame(n_rep1 = ntaxa(psLst[[col1]]),
                           n_rep2 = ntaxa(psLst[[col2]]),
                           n_intersect = length(intersect(psLst[[col1]] %>% taxa_names(), psLst[[col2]] %>% taxa_names())),
                           n_union = length(union(psLst[[col1]] %>% taxa_names(), psLst[[col2]] %>% taxa_names())))
      df_vec <- df_vec %>% 
        mutate(Ratio = n_intersect/n_union)
      rownames(df_vec) <- nm
      df_ratio[nrow(df_ratio)+1, ] <- df_vec
  }
  return(df_ratio)
}
...
Samplewise_OverlapOverall_Ratio <- function(psLst){
  ...
  df_num <- df_taxa %>% 
    ...
    select(AN_NR, Raw, Restrictive, Decontam, SCRuB, Nj)
  return(df_num)
}
...
##### Original Phyloseq, without taxa_agg or rare taxa filtering

Overall table

```{r}
tab <- Ratio_table_calc(psLst)
print(tab)
```

Samplewise and barplot

```{r}
df_num <- Samplewise_OverlapOverall_Ratio(psLst = psLst)
df_num <- df_num %>% 
  pivot_longer(cols = -AN_NR, names_to = "Methods", values_to = "Val") 
df_num$Methods <- factor(df_num$Methods, levels = c("Nj", "Restrictive", "SCRuB", "Decontam", "Raw")) 
df_num %>% 
  AlphaPlot_Violin_Wilcox(SampleID = "AN_NR", strata = "Methods", val = "Val", y_label = "Methods", subtitle = "Higher is better", comparisons = list(c("Nj", "Restrictive"), c("Nj", "SCRuB"), c("Nj", "Decontam"), c("Nj", "Raw")), alternative = "greater", p.adjust.method = "BH")
```

Pairwise p-value table

```{r}
tab <- pairwise_WT_Method(df_longer = df_num, alternative = "greater")
print(tab)
```
...
```{r}
# funct_overlap <- function(ps1, ps2, ps3, ps4, ps5, 
#                           rep1, rep2, rep3, rep4, rep5){
#   df_overlap <- tibble(Method = c("Raw", "Restrictive", "Decontam", "SCRuB", "Nj"),
#                      n_rep1 = 0, n_rep2 = 0,
#                      n_intersect = 0, n_union = 0)
#   pseq_raw <- ps1
#   pseq_restrictive <- ps2
#   pseq_decontam <- ps3
#   pseq_SCRuB <- ps4
#   pseq_Nj <- ps5
#   rep_raw <- rep1
#   rep_restrictive <- rep2
#   rep_decontam <- rep3
#   rep_SCRuB <- rep4
#   rep_Nj <- rep5
#   df_overlap$n_rep1 <- c(ntaxa(pseq_raw), ntaxa(pseq_restrictive), ntaxa(pseq_decontam), ntaxa(pseq_SCRuB), ntaxa(pseq_Nj))
#   df_overlap$n_rep2 <- c(ntaxa(rep_raw), ntaxa(rep_restrictive), ntaxa(rep_decontam), ntaxa(rep_SCRuB), ntaxa(rep_Nj))
#   df_overlap$n_intersect <- c(
#     length(intersect(taxa_names(pseq_raw), taxa_names(rep_raw))),
#     length(intersect(taxa_names(pseq_restrictive), taxa_names(rep_restrictive))),
#     length(intersect(taxa_names(pseq_decontam), taxa_names(rep_decontam))),
#     length(intersect(taxa_names(pseq_SCRuB), taxa_names(rep_SCRuB))),
#     length(intersect(taxa_names(pseq_Nj), taxa_names(rep_Nj)))
#   )
#   df_overlap$n_union <- c(
#     length(union(taxa_names(pseq_raw), taxa_names(rep_raw))),
#     length(union(taxa_names(pseq_restrictive), taxa_names(rep_restrictive))),
#     length(union(taxa_names(pseq_decontam), taxa_names(rep_decontam))),
#     length(union(taxa_names(pseq_SCRuB), taxa_names(rep_SCRuB))),
#     length(union(taxa_names(pseq_Nj), taxa_names(rep_Nj)))
#   )
#   df_overlap <- df_overlap %>% 
#     mutate(intersect_per_union = n_intersect/n_union)
#   return(df_overlap)
# }
...
min_prev <- 0.1
min_abund <- 0.001
psLst_reduce <- Reduce_phyloseq(psLst, min_abund = min_abund, min_prev = min_prev)
```

Overall table

```{r}
tab <- Ratio_table_calc(psLst_reduce)
print(tab)
```

Samplewise - barplot and square table

```{r}
df_num <- Samplewise_OverlapOverall_Ratio(psLst = psLst_reduce)
df_num <- df_num %>% 
  pivot_longer(cols = -AN_NR, names_to = "Methods", values_to = "Val") 
df_num$Methods <- factor(df_num$Methods, levels = c("Nj", "Restrictive", "SCRuB", "Decontam", "Raw")) 
df_num %>% 
...
min_prev <- 0.3
min_abund <- 0.0025
psLst_reduce <- Reduce_phyloseq(psLst = psLst, rank_level = "species", min_prev = min_prev, min_abund = min_abund)
```
...
min_abund <- 0.0001
rnk_tax <- "genus"
psLst_reduce <- Reduce_phyloseq(psLst = psLst, rank_level = rnk_tax, min_prev = min_prev, min_abund = min_abund)
...
Get overlap species name

```{r}
tmp_Nj <- psLst$Nj_R1 %>% 
  prune_taxa(taxa_names(.) %in% taxa_names(psLst$Nj_R2), .) %>% 
  microViz::tax_mutate(Nj = "YES") %>% 
  tax_tibble()
tmp_SCRuB <- psLst$SCRuB_R1 %>% 
  prune_taxa(taxa_names(.) %in% taxa_names(psLst$SCRuB_R2), .) %>% 
  microViz::tax_mutate(SCRuB = "YES") %>% 
  tax_tibble()
tmp_Decontam <- psLst$Decontam_R1 %>% 
  prune_taxa(taxa_names(.) %in% taxa_names(psLst$Decontam_R2), .) %>% 
  microViz::tax_mutate(Decontam = "YES") %>% 
  tax_tibble()
tmp_Restrictive <- psLst$Restrictive_R1 %>% 
  prune_taxa(taxa_names(.) %in% taxa_names(psLst$Restrictive_R2), .) %>% 
  microViz::tax_mutate(Restrictive = "YES") %>% 
  tax_tibble()
df_taxa <- tmp_Nj %>% 
  left_join(., y = tmp_SCRuB, by = c("FeatureID", "superkingdom", "phylum", "class", "order", "family", "genus", "species")) %>% 
  left_join(., y = tmp_Decontam, by = c("FeatureID", "superkingdom", "phylum", "class", "order", "family", "genus", "species")) %>% 
  left_join(., y = tmp_Restrictive, by = c("FeatureID", "superkingdom", "phylum", "class", "order", "family", "genus", "species")) %>% 
  arrange(genus, species) %>% 
  select(-FeatureID)
library(openxlsx)
openxlsx::write.xlsx(df_taxa, file = "Nj_overlap_taxa.xlsx")
```

### 3.3 Investigate microbiome in overlap, each method separately

-   Columns: Raw and decontamination methods
-   Rows: Replica 1 +2, overlap, and number of taxa in each

Edit meta data

```{r}
## Name of sample: Method_Replica_AN
...
pseq_raw <- pseq_raw %>% 
  rename_samples(m_Replica = "R1", m_Method = "Raw")
pseq_Nj <- pseq_Nj %>% 
  rename_samples(m_Replica = "R1", m_Method = "Nj")
pseq_restrictive <- pseq_restrictive %>% 
  rename_samples(m_Replica = "R1", m_Method = "Restrictive")
pseq_decontam <- pseq_decontam %>% 
  rename_samples(m_Replica = "R1", m_Method = "Decontam")
pseq_SCRuB <- pseq_SCRuB %>% 
  rename_samples(m_Replica = "R1", m_Method = "SCRuB")
##-----------------------------
rep_raw <- rep_raw %>% 
  rename_samples(m_Replica = "R2", m_Method = "Raw")
rep_Nj <- rep_Nj %>% 
  rename_samples(m_Replica = "R2", m_Method = "Nj")
rep_restrictive <- rep_restrictive %>% 
  rename_samples(m_Replica = "R2", m_Method = "Restrictive")
rep_decontam <- rep_decontam %>% 
  rename_samples(m_Replica = "R2", m_Method = "Decontam")
rep_SCRuB <- rep_SCRuB %>% 
  rename_samples(m_Replica = "R2", m_Method = "SCRuB")

##--------------------------------
ps_all <- merge_phyloseq(pseq_raw, pseq_restrictive, pseq_decontam, pseq_SCRuB, pseq_Nj,
                         rep_raw, rep_restrictive, rep_decontam, rep_SCRuB, rep_Nj)
  # merge_phyloseq(pseq_raw, pseq_Nj) %>% 
  # merge_phyloseq(rep_raw) %>% 
  # merge_phyloseq(rep_Nj) %>% 
...
# shared_taxa_raw <- intersect(taxa_names(pseq_raw), taxa_names(rep_raw))
# ol_raw <- merge_phyloseq(pseq_raw %>% prune_taxa(taxa_names(.) %in% shared_taxa_raw, .) %>% microbiome::transform(transform = "compositional"),
#                          rep_raw %>% prune_taxa(taxa_names(.) %in% shared_taxa_raw, .) %>% microbiome::transform(transform = "compositional")) %>% 
#   merge_samples(group = "AN_NR")
# AN_lst <- sample_names(ol_raw)
# ol_raw <- ol_raw %>% 
#   ps_mutate(Replica = "Overlap", Method = "Raw", AN_NR = AN_lst) %>% 
#   ps_mutate(Group = sprintf("%s_%s", Method, Replica)) %>% 
#   ps_mutate(NewID = sprintf("%s_%s", Group, AN_NR)) %>% ps_arrange(AN_NR)
# sample_names(ol_raw) <- meta(ol_raw)[["NewID"]]

ol_raw <- overlap_ps(pseq_raw, rep_raw, m_group = "AN_NR")
ol_restrictive <- overlap_ps(pseq_restrictive, rep_restrictive, m_group = "AN_NR")
ol_decontam <- overlap_ps(pseq_decontam, rep_decontam, m_group = "AN_NR")
ol_SCRuB <- overlap_ps(pseq_SCRuB, rep_SCRuB, m_group = "AN_NR")
ol_Nj <- overlap_ps(pseq_Nj, rep_Nj, m_group = "AN_NR")
ps_all <- ps_all %>% 
  merge_phyloseq(ol_raw, ol_restrictive, ol_decontam, ol_SCRuB, ol_Nj)
```
...
ps_2 %>% CompositionalPlot_Viz_v04(my_facet = "Method", supper_rank = "family", smaller_rank = "genus", each_num = 4, n_super_rank = 4)
  # comp_barplot(tax_level = "genus", n_taxa = 12, bar_width = 0.8) +
  # coord_flip() + labs(x = NULL, y = NULL)
  #CompositionalPlot_Viz_v04(my_facet = "Method")
```
...
ps_2 %>% 
   comp_barplot(
    tax_level = "family", n_taxa = 15, bar_width = 0.7, sample_order = "asis", # don't sort
    x = "Replica" # x argument is available since microViz 0.9.7
  ) +
  facet_wrap(
    facets = vars(Method),
...
```{r}
## oberlap of decontam, SCRuB and Nj, for each rep1 and rep2

# shared_taxa_rep1 <- taxa_names(pseq_decontam) %>% 
#   intersect(taxa_names(pseq_SCRuB)) %>% 
#   intersect(taxa_names(pseq_Nj))
# shared_taxa_rep2 <- taxa_names(rep_decontam) %>% 
#   intersect(taxa_names(rep_SCRuB)) %>% 
#   intersect(taxa_names(rep_Nj))
# 
# ThreeMed_rep1 <- merge_phyloseq(
#   pseq_decontam %>% prune_taxa(taxa_names(.) %in% shared_taxa_rep1, .),
#   pseq_SCRuB %>% prune_taxa(taxa_names(.) %in% shared_taxa_rep1, .),
#   pseq_Nj %>% prune_taxa(taxa_names(.) %in% shared_taxa_rep1, .)
# )
# ThreeMed_rep2 <- merge_phyloseq(
#   rep_decontam %>% prune_taxa(taxa_names(.) %in% shared_taxa_rep2, .),
#   rep_SCRuB %>% prune_taxa(taxa_names(.) %in% shared_taxa_rep2, .),
#   rep_Nj %>% prune_taxa(taxa_names(.) %in% shared_taxa_rep2, .)
# )
# 
# df_meta_1 <- meta(ThreeMed_rep1) %>% 
#   group_by(AN_NR) %>% 
#   summarise(
#     across(
#       everything(),
...
#   remove_rownames() %>% 
#   column_to_rownames(var = "NewID")
# ThreeMed_rep1 <- merge_samples(ThreeMed_rep1, group = "AN_NR")
# ThreeMed_rep2 <- merge_samples(ThreeMed_rep2, group = "AN_NR")
```
