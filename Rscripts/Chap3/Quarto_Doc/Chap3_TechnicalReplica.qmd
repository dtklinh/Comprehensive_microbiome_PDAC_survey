...
```{r}
# model <- lmer(formula = Val ~ Methods + (1|AN_NR), data = df_longer_1)
# emm <- emmeans(model, specs = pairwise ~ Methods)
# contr <- contrast(emm, method = "pairwise", adjust = "None")
df_longer <- df_longer %>% 
  mutate(Val_NegLog = -log(Val))
res <- wilcox_test(data = df_longer, formula = Val_NegLog ~ Methods, paired = T, p.adjust.method = "none", comparisons = list(c("Raw", "Restrictive"), c("Raw", "Decontam"), c("Raw", "SCRuB"), c("Raw", "Nj")), alternative = "greater") %>% add_xy_position()
pairwise_wilcox_test()

p1 <- ggplot (df_longer, aes(x = Methods, y = Val_NegLog, fill=Methods))+ 
    #geom_boxplot()+
    geom_violin() +
    geom_boxplot(width=0.2) +
...
p1 + stat_pvalue_manual(res, label = "p", inherit.aes = FALSE, tip.length = 0.01)
```
