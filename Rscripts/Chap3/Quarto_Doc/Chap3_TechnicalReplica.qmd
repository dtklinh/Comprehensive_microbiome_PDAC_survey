...
AN_lst <- meta(pseq_raw)[["AN_NR"]]
raw_core_taxa <- intersect(taxa_names(pseq_raw), taxa_names(rep_raw))
ol_raw <- merge_phyloseq(pseq_raw, rep_raw) %>% 
  prune_taxa(taxa_names(.) %in% raw_core_taxa, .) %>% 
  merge_samples(group = "AN_NR") %>% 
  ps_mutate(Replica = "Overlap", Method = "Raw") %>% 
  ps_mutate(Group = sprintf("%s_%s", Method, Replica)) %>% 
  ps_mutate(NewID = sprintf("%s_%s", Group, AN_NR)) %>% 
  ps_arrange(AN_NR)
sample_names(ol_raw) <- meta(ol_raw)[["NewID"]]
sample_data(ol_raw)$AN_NR <- sample_names(ol_raw)
```
