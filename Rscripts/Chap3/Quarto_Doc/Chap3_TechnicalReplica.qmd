...
### 3.2 Investigate microbiome in overlap, each method separately
...
ps_all <- merge_phyloseq(pseq_raw, pseq_Nj) %>% 
...
ps_all <- ps_all %>% 
  merge_phyloseq(ol_raw) %>% 
  merge_phyloseq(ol_Nj)
```

Test for Raw and Nj

```{r}
CompositionalPlot_Viz_v04(PhyloseqObj = ps_all, my_facet = "")
```
