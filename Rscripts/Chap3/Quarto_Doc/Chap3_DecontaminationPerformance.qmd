...
## Venn diagram with control of species names
my_Venn_Plot2 <- function(lst_3sets){
  ## NCT_idx = 3
  ## add item to the list
  A_name <- names(lst_3sets)[[1]]
  B_name <- names(lst_3sets)[[2]]
  C_name <- names(lst_3sets)[[3]]
  int_ABC <- intersect(lst_3sets[[1]], lst_3sets[[2]]) %>% intersect(., lst_3sets[[3]])
  A_only <- setdiff(lst_3sets[[1]], lst_3sets[[2]]) %>% setdiff(., lst_3sets[[3]])
  B_only <- setdiff(lst_3sets[[2]], lst_3sets[[1]]) %>% setdiff(., lst_3sets[[3]])
  int_AB <- intersect(lst_3sets[[1]], lst_3sets[[2]]) %>% setdiff(., int_ABC)
  int_AC <- intersect(lst_3sets[[1]], lst_3sets[[3]]) %>% setdiff(., int_ABC)
  int_BC <- intersect(lst_3sets[[2]], lst_3sets[[3]]) %>% setdiff(., int_ABC)
  ## venn plot
  venn_plot <- draw.triple.venn(
    area1 = length(lst_3sets[[1]]), area2 = length(lst_3sets[[2]]), area3 = length(lst_3sets[[3]]),
    n12 = length(int_AB) + length(int_ABC), 
    n23 = length(int_BC) + length(int_ABC), 
    n13 = length(int_AC) + length(int_ABC), 
    n123 = length(int_ABC),
    category = c(A_name, B_name, C_name),
    fill = c("skyblue", "pink", "palegreen"),
    alpha = 0.5,
    lty = "blank",
    cex = 1.5,            # Size of numbers
    cat.cex = 1.2,      # Size of Group Titles
    cat.col = c("blue", "red", "darkgreen")
  )
  # Setup Layout (Venn on Left, Legend on Right)
  grid.newpage()
  ...
  current_y <- 0.95  # Start at the top

  draw_section <- function(title, species_list, title_col, y_in) {
    # Draw Title
    grid.text(title, x = 0, y = y_in, just = "left", 
              gp = gpar(col = title_col, fontface = "bold", fontsize = 10))
    
    # Prepare and wrap species text
    species_text <- paste(species_list, collapse = ", ")
    wrapped <- strwrap(species_text, width = 65) # Wrap text to fit width
    
    # Draw each line of wrapped text
    new_y <- y_in - 0.03
    for(line in wrapped) {
      grid.text(line, x = 0.02, y = new_y, just = "left", 
                gp = gpar(col = "black", fontsize = 9, fontitalic = TRUE))
      new_y <- new_y - 0.025 # Move down for next line
    }
    
    return(new_y - 0.02) # Return ending position with a small buffer
  }


  # Positioning the 6 categories
  current_y <- draw_section(sprintf("Unique to %s", A_name), A_only, "blue", current_y)
  current_y <- draw_section(sprintf("Unique to %s", B_name), B_only, "red", current_y)
  #draw_sub_legend("Unique to C", set_c_only, "darkgreen", 0.69)
  current_y <- draw_section(sprintf("%s & %s Intersection", A_name, B_name), int_AB, "purple", current_y)
  current_y <- draw_section(sprintf("%s & %s Intersection", B_name, C_name), int_BC, "brown", current_y)
  current_y <- draw_section(sprintf("%s & %s Intersection", A_name, C_name), int_AC, "darkcyan", current_y)
  draw_section("Common to All", int_ABC, "black", current_y)
...
AlphaPlot_Violin_Wilcox(df, "AN_NR", "Method", "Val", "Shannon Index")
```
...
AlphaPlot_Violin_Wilcox(df, "AN_NR", "Method", "Val", "invsimpson Index")
```
...
Integrate info of serial NCT into overlap analyses. A list of species in NCT samples with at least 25% prevalence.
...
```{r}
X <- list(
  SCRuB = tax_top(pseq_SCRuB, n=15, rank = "species", by = sum),
  Raw = tax_top(pseq_raw, n=15, rank = "species", by = sum),
  NCT = gsub("_", " ", lst_NCT$taxa)
)
my_Venn_Plot2(X)
```
...
```{r}
X <- list(
  SCRuB = tax_top(pseq_SCRuB, n=15, rank = "species", by = prev),
  Raw = tax_top(pseq_raw, n=15, rank = "species", by = prev),
  NCT = gsub("_", " ", lst_NCT$taxa)
)
my_Venn_Plot2(X)
```
...
```{r}
X <- list(
  Nj = tax_top(pseq_Nj, n=15, rank = "species", by = sum),
  Raw = tax_top(pseq_raw, n=15, rank = "species", by = sum),
  NCT = gsub("_", " ", lst_NCT$taxa)
)
my_Venn_Plot2(X)
...
```{r}
X <- list(
  Nj = tax_top(pseq_Nj, n=15, rank = "species", by = prev),
  Raw = tax_top(pseq_raw, n=15, rank = "species", by = prev),
  NCT = gsub("_", " ", lst_NCT$taxa)
)
my_Venn_Plot2(X)
...
For each method, only keep the significant by the following rules:

-   minimum total abundance is at lest 1e-4

-   prevalence is at least 25%

For the NCT samples, minimum abundance is at least 1e-6, and prevalence is at least 10%.

```{r}
X <- list(
  Raw = pseq_raw %>% tax_filter(min_prevalence = 0.25, min_total_abundance = 1e-4, undetected = 0, use_counts = FALSE) %>% taxa_names(),
  decon = pseq_decontam %>% tax_filter(min_prevalence = 0.25, min_total_abundance = 1e-4, undetected = 0, use_counts = FALSE) %>% taxa_names(),
  SCRuB = pseq_SCRuB %>% tax_filter(min_prevalence = 0.25, min_total_abundance = 1e-4, undetected = 0, use_counts = FALSE) %>% taxa_names(),
  Nj = pseq_Nj %>% tax_filter(min_prevalence = 0.25, min_total_abundance = 1e-4, undetected = 0, use_counts = FALSE) %>% taxa_names(),
  NCT = pseq_NCT %>% tax_filter(min_prevalence = 0.10, min_total_abundance = 1e-6, undetected = 0, use_counts = FALSE) %>% taxa_names()
)
## pick some names for represented
Nj_notNCT <- pseq_Nj %>% 
  prune_taxa(!taxa_names(.) %in% X$NCT, .) %>% 
  tax_top(n = 10, by = sum, rank = "species")
Scrub_notNCT <-  pseq_SCRuB %>% 
  prune_taxa(!taxa_names(.) %in% X$NCT, .) %>% 
  tax_top(n = 10, by = sum, rank = "species")
decon_notNCT <- pseq_decontam %>% 
  prune_taxa(!taxa_names(.) %in% X$NCT, .) %>% 
  tax_top(n = 10, by = sum, rank = "species")
Raw_notNCT <- pseq_raw %>% 
  prune_taxa(!taxa_names(.) %in% X$NCT, .) %>% 
...
plt <- venn.diagram(X,
                    filename = NULL,
                    category = names(X),
                    fill = c("skyblue", "pink", "palegreen", "cyan", "yellow"),
                    alpha = 0.5,
                    lty = "blank",
                    cex = 1.5,            # Size of numbers
                    cat.cex = 1.2)
...
grid.newpage()
  ...
  # Positioning the 6 categories
  current_y <- draw_section(sprintf("Raw not in NCT"), Raw_notNCT, "blue", current_y)
  current_y <- draw_section(sprintf("Decon not in NCT"), decon_notNCT, "red", current_y)
  #draw_sub_legend("Unique to C", set_c_only, "darkgreen", 0.69)
  current_y <- draw_section(sprintf("SCRuB not in NCT"), Scrub_notNCT, "green", current_y)
  current_y <- draw_section(sprintf("Nj not in NCT"), Nj_notNCT, "cyan", current_y)
  #current_y <- draw_section(sprintf("%s & %s Intersection", A_name, C_name), int_AC, "darkcyan", current_y)
  #draw_section("Common to All", int_ABC, "black", current_y)
...
We use upset plot to illustrate the overlap of more than 3 sets.
...
Integrate contaminant list in NCT.
...
p <- upset(
  ...
  annotations = list(
    "reads" = (
      ggplot(mapping=aes(y = Reads, fill=class))
      + geom_bar(stat = "summary", fun = "sum", position='fill')
      + scale_y_continuous(labels=scales::percent_format())
      + scale_fill_manual(values= Kolors
                          #c('R'='#E41A1C', 'PG'='#377EB8', 'PG-13'='#4DAF4A', 'NC-17'='#FF7F00')
    ...
    'class' = list(
      ...
      geom = list(
        geom_bar(stat='count', position='fill', na.rm=TRUE),
        geom_text(
          aes(label=ifelse(class == 'Alphaproteobacteria', 'Alpha.', NA)),
          stat='count',
          position=position_fill(vjust = .5),
          na.rm=TRUE
        ),
...
p <- upset(
  ...
  annotations = list(
    "reads" = (
      ggplot(mapping=aes(y = Reads, fill=phylum))
      + geom_bar(stat = "summary", fun = "sum", position='fill')
      + scale_y_continuous(labels=scales::percent_format())
      + scale_fill_manual(values= Kolors
                          #c('R'='#E41A1C', 'PG'='#377EB8', 'PG-13'='#4DAF4A', 'NC-17'='#FF7F00')
