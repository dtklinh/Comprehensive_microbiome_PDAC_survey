...
# popViewport()
# 
### Venn diagram with only few species names
X <- list(Raw = pseq_raw %>% tax_filter(undetected = 0, min_total_abundance = 1e-4, min_prevalence = 0.25, tax_level = "species", use_counts = F) %>% taxa_names(),
          decon = pseq_decontam %>% tax_filter(undetected = 0, min_total_abundance = 1e-4, min_prevalence = 0.25, tax_level = "species", use_counts = F) %>% taxa_names(),
          SCRuB = pseq_SCRuB %>% tax_filter(undetected = 0, min_total_abundance = 1e-4, min_prevalence = 0.25, tax_level = "species", use_counts = F) %>% taxa_names(),
          Nj = pseq_Nj %>% tax_filter(undetected = 0, min_total_abundance = 1e-4, min_prevalence = 0.25, tax_level = "species", use_counts = F) %>% taxa_names(),
          NCT = pseq_NCT %>% tax_filter(undetected = 0, min_total_abundance = 1e-6, min_prevalence = 0.1, tax_level = "species", use_counts = F) %>% taxa_names())
plt <- venn.diagram(X, filename = NULL,
             category = c("Raw", "decon", "SCRuB", "Nj", "NCT"),
             fill = Kolors[1:5],
             alpha = 0.5,
             lty = "blank",
             cex = 2,            # Size of numbers
...
## inspect tax_filter function
ps1 <- readRDS("data/Chap1/NCTs_v7.rds")
ps2 <- readRDS("data/Chap1/NCTs_v7_wrench.rds")

tax_filter(ps2, min_total_abundance = 0, min_prevalence = 1, use_counts = F)
####

ps_rel <- transform_sample_counts(ps1, function(x) x / sum(x))
